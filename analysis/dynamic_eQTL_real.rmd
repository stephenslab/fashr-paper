...
```{r some_genes_highlighted_original , echo=FALSE}
...
```{r most_significant_SCN5A , echo=FALSE}
...
# find fash lfdr for these genes
all_genes_nonunique <- sapply(strsplit(names(datasets), "_"), "[[", 1)
index_missed_genes <- which(all_genes_nonunique %in% gene_missed)
# get the minimum lfdr for each gene
min_lfdr_missed_genes <- sapply(gene_missed, function(gene) {
  indices <- which(all_genes_nonunique == gene)
  min(fash_fit1_update$lfdr[indices])
})
# pick the four genes with smallest min lfdr
gene_missed <- names(sort(min_lfdr_missed_genes))
