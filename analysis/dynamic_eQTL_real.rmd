...
```{r}
genes_highlighted1 <- unique(pair_tbl$ens_id[pair_tbl$idx %in% fash_highlighted1])
...
sym2ens <- symbol_to_ens(selected_gene_fashr_only, file.path(result_dir, "cache_sym2ens_only_fash_least.rds"))
gene_ens <- sym2ens$ensembl_gene_id[match(selected_gene_fashr_only, sym2ens$hgnc_symbol)]

# choose, for each gene, the pair among pairs_pass_filter with max adjusted lfdr
idxs <- map_int(gene_ens, function(gid){
...
qt_threshold <- quantile(fash_fit1_update$lfdr[test1$fdr_results$index[test1$fdr_results$FDR <= alpha]], 0.1)
...
strober_best <- full_join(
  strober_linear_highlighted  %>% group_by(ensamble_id) %>% slice_min(pvalue, n = 1, with_ties = FALSE) %>% ungroup() %>% 
    select(ensamble_id, rs_id, pvalue) %>% rename(rs_id_linear = rs_id, p_linear = pvalue),
  strober_nonlinear_highlighted %>% group_by(ensamble_id) %>% slice_min(pvalue, n = 1, with_ties = FALSE) %>% ungroup() %>% 
    select(ensamble_id, rs_id, pvalue) %>% rename(rs_id_nonlinear = rs_id, p_nonlinear = pvalue),
  by = "ensamble_id"
) %>%
...
idxs_early <- head(early_indices, 4)
...
sym2ens_sw <- symbol_to_ens(selected_switch_genes_symbols, file.path(result_dir, "cache_sym2ens_switch.rds"))
selected_switch_genes <- sym2ens_sw$ensembl_gene_id[match(selected_switch_genes_symbols, sym2ens_sw$hgnc_symbol)]
```

Let's take a look at the top-ranked switch dynamic eQTLs:

```{r switch_examples , echo=FALSE}
# for each selected switch gene, pick the first significant index in that gene among switch_indices
idxs_sw <- map_int(selected_switch_genes, function(gid){
  cand <- pair_tbl %>% filter(ens_id == gid, idx %in% switch_indices) %>% pull(idx)
  if (length(cand) == 0) return(NA_integer_)
  cand[which.min(fash_fit1_update$lfdr[cand])]
})
idxs_sw <- idxs_sw[!is.na(idxs_sw)]
