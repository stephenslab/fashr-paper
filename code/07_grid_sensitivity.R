...
fash_fit1_finer <- fash(Y = "beta", smooth_var = "time", S = "SE", data_list = datasets,
                  num_basis = 20, order = 1, betaprec = 0,
                  pred_step = 1, penalty = 10, grid = finer_grid,
                  num_cores = num_cores, verbose = TRUE)
save(fash_fit1_finer, file = "./results/fash_fit1_finer_all.RData")


### Sparser grid:
log_prec <- seq(0,10, by = 0.5)
sparse_grid <- sort(c(0, exp(-0.5*log_prec)))
fash_fit1_sparse <- fash(Y = "beta", smooth_var = "time", S = "SE", data_list = datasets,
                  num_basis = 20, order = 1, betaprec = 0,
                  pred_step = 1, penalty = 10, grid = sparse_grid,
                  num_cores = num_cores, verbose = TRUE)
save(fash_fit1_sparse, file = "./results/fash_fit1_sparse_all.RData")
